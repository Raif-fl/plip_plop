...
def save_pair_dataset(signal_A, signal_B, output_file, rbp_A, rbp_B, cell_type):
    ...
    np.savez_compressed(output_file, signals=signals, chrom=np.array([x["chr"] for x in signal_A]),
                        start=np.array([x["start"] for x in signal_A]), end=np.array([x["end"] for x in signal_A]),
                        strand=np.array([x["strand"] for x in signal_A]), region_id=np.array([x["region_id"] for x in signal_A]),
                        block_number=np.array([x["block_number"] for x in signal_A]), rbp_A=np.array(rbp_A),
                        rbp_B=np.array(rbp_B), cell_type=np.array(cell_type))
